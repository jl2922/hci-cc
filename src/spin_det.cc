...
SpinDet::SpinDet(const SpinDet &spin_det) { orbs = spin_det.orbs; }
...
int SpinDet::get_n_elecs() const { return orbs.count(); }
...
int SpinDet::get_n_orbs() const { return orbs.size(); }

bool SpinDet::is_zero() const { return orbs.none(); }

void SpinDet::resize(const int n_orbs) { orbs.resize(n_orbs); }
